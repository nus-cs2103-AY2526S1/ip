...
public class Parser {
    ...
    public static Parsed parse(String input) throws TsayYongBotException {
        ...
        if (mt.matches()) {
            ...
            if (desc.isEmpty()) {
                throw new TsayYongBotException("Usage: todo <description>");
            }
        ...
        if (md.matches()) {
            ...
            if (desc.isEmpty() || by.isEmpty()) {
                throw new TsayYongBotException("Usage: deadline <desc> /by <when>");
            }
        ...
        if (me.matches()) {
            ...
            if (desc.isEmpty() || from.isEmpty() || to.isEmpty()) {
                throw new TsayYongBotException("Usage: event <desc> /from <start> /to <end>");
            }
            return new Parsed(CommandType.EVENT, desc, null, from, to, null);
        }

        throw new TsayYongBotException("Unknown command. Try: todo, deadline, event, list, mark, unmark, delete, bye.");
    }
}
