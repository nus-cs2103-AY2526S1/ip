...
public class InvalidTaskIndexException extends EvansBotException {
    ...
    public InvalidTaskIndexException(int max) {
        super(max == 0
                ? "Error! There are currently no tasks inside the task list. Please add a task first!"
                : "Error! There are only " + max + " tasks in the list! Please choose a number between 1 to " + max);
        this.maxIndex = max;
    }
